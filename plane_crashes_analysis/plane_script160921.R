...
#plotting aircraft with highest crash count, highest survival, and highest fatality rates
crash_data_v2 %>%
  group_by(ac_type) %>%
  summarise(number_of_crashes = n()) %>%
  arrange(-number_of_crashes) %>%
  head(n = 10) %>%
  ggplot(aes(x = reorder(ac_type, number_of_crashes), y = number_of_crashes)) +
  geom_col() +
  coord_flip() +
...
crash_data_v2 %>%
  ...
  arrange(-number_of_crashes) %>%
  head(n = 10) %>%
  ggplot(aes(x = reorder(ac_type, number_of_crashes), y = mean_survival)) +
  geom_col() +
  coord_flip() +
...
crash_data_v2 %>%
  ...
  summarise(mean_survival = mean(survival_rate), mean_survival_rate, deviation = mean_survival - mean_survival_rate)
